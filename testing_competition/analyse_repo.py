...
RE_PARSE_BLAME = re.compile(r'(?P<commit_hash>.+)\((?P<commit_author>[\w0-1]+) (?P<commit_date>\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}) \+0200\s*(?P<line_number>\d+)\)(?P<content>.*)')
...
def get_git_blame(path_test: Path) -> List[BlameLine]:
    ...
    for raw_blame in raw_blame_lines:
        match_blame = RE_PARSE_BLAME.match(raw_blame).groupdict()
